...
- **Subagent rollups:** when a Claude Code session has sidechain/subagent transcripts under its own transcript directory, `watch` shows one dim `↳ agents:` line below the parent session, for example `1 active · 2 done · latest: Design migration`. It does not expand subagent logs by default — this is just enough hierarchy to show hidden parallel work without turning the dashboard into a log explorer.
...
4. The dashboard pane loops every 0.5s. It snapshots the log under a shared `flock`, runs the reducer in-process to produce `current.json`, reads additive Claude Code display metadata (`/rename`, `/color`) plus cached native `away_summary` recaps and subagent transcript rollups, renders the frame, and rings the bell on new `waiting_input` sessions.
5. There is no daemon. No IPC. No database. If the dashboard dies, `events.jsonl` keeps growing. On restart, the reducer rebuilds the state from the log; display metadata, recaps, and subagent rollups are re-read from Claude Code's own files.
...
**No `↳ agents:` line yet** — subagent rollups only appear when Claude Code has written sidechain files under `~/.claude/projects/<cwd-slug>/<parent-session-id>/subagents/`. Regular single-agent sessions omit the line. The active/done split is intentionally heuristic in the MVP: recently modified subagent transcripts count as active; older ones count as done.
